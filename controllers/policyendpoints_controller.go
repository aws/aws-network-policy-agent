...
func (r *PolicyEndpointsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	clog.Infof("Received a new reconcile request %v", req)
	if err := r.reconcile(ctx, req); err != nil {
		clog.Errorf("Reconcile error: %v", err)
...
func (r *PolicyEndpointsReconciler) reconcile(ctx context.Context, req ctrl.Request) error {
	...
	if err := r.k8sClient.Get(ctx, req.NamespacedName, policyEndpoint); err != nil {
		...
		clog.Errorf("Unable to get policy endpoint spec for policyendpoint %s: %v", req.NamespacedName, err)
		return err
	}
...
func (r *PolicyEndpointsReconciler) cleanUpPolicyEndpoint(ctx context.Context, req ctrl.Request) error {
	clog := logger.FromContext(ctx)
	clog.Infof("Clean Up PolicyEndpoint resources for name: %s", req.NamespacedName.Name)
	...
	clog.Infof("cleanUpPolicyEndpoint: Pods to cleanup - %d and Pods to be updated - %d", len(podsToBeCleanedUp), len(targetPods))

	// targetPods are pods which would need map update
	if len(targetPods) > 0 {
		clog.Debug("Updating active pods...")
		err := r.updatePolicyEnforcementStatusForPods(ctx, req.NamespacedName.Name, targetPods, podIdentifiers, false)
		if err != nil {
			clog.Errorf("failed to update bpf probes for policy endpoint %s : %v", req.NamespacedName.Name, err)
			return err
		}
	...
	if len(podsToBeCleanedUp) > 0 {
		clog.Debug("Cleaning up current policy against below pods..")
		err := r.updatePolicyEnforcementStatusForPods(ctx, req.NamespacedName.Name, podsToBeCleanedUp, podIdentifiers, true)
		if err != nil {
			clog.Errorf("failed to clean up bpf probes for policy endpoint %s : %v", req.NamespacedName.Name, err)
			return err
		}
		duration := msSince(start)
		policyTearDownLatency.WithLabelValues(req.NamespacedName.Name, req.NamespacedName.Namespace).Observe(duration)
...
func (r *PolicyEndpointsReconciler) updatePolicyEnforcementStatusForPods(ctx context.Context, policyEndpointName string,
	...
	for _, targetPod := range targetPods {
		...
		clog.Infof("Updating Pod: Name: %s Namespace: %s PodIdentifier: %s", targetPod.Name, targetPod.Namespace, podIdentifier)

		cleanupErr := r.cleanupPod(ctx, targetPod, policyEndpointName, isDeleteFlow)
		if cleanupErr != nil {
			clog.Errorf("Cleanup/Update unsuccessful for Pod Name: %s Namespace: %s ", targetPod.Name, targetPod.Namespace)
...
func (r *PolicyEndpointsReconciler) reconcilePolicyEndpoint(ctx context.Context,
	...
	clog := logger.FromContext(ctx)
	clog.Infof("Processing Policy Endpoint  Name: %s Namespace %s", policyEndpoint.Name, policyEndpoint.Namespace)
	start := time.Now()
	...
	if err != nil {
		clog.Errorf("failed to update policy enforcement status for existing pods: %v", err)
		return err
	}
	...
	for podIdentifier := range podIdentifiers {
		...
		if err != nil {
			clog.Errorf("Error Parsing policy Endpoint resource %s: %v", policyEndpoint.Name, err)
			return err
		}

		if len(ingressRules) == 0 && !isIngressIsolated {
			//Add allow-all entry to Ingress rule set
			clog.Info("No Ingress rules and no ingress isolation - Appending catch all entry")
			r.addCatchAllEntry(&ingressRules)
		}

		if len(egressRules) == 0 && !isEgressIsolated {
			//Add allow-all entry to Egress rule set
			clog.Info("No Egress rules and no egress isolation - Appending catch all entry")
		...
		if err != nil {
			clog.Errorf("Error configuring eBPF Probes %v", err)
		}
		duration := msSince(start)
	...
	r.observePolicyProgrammingLatency(ctx, policyEndpoint)
...
func (r *PolicyEndpointsReconciler) observePolicyProgrammingLatency(ctx context.Context, pe *policyk8sawsv1.PolicyEndpoint) {
	...
	clog.Debugf("E2E policy programming latency: %.3fs for PE %s/%s", latency, pe.Namespace, pe.Name)
}
...
func (r *PolicyEndpointsReconciler) configureeBPFProbes(ctx context.Context, podIdentifier string,
	...
	for _, pod := range targetPods {
		...
		if currentPodIdentifier != podIdentifier {
			clog.Debugf("Target Pod doesn't belong to the current pod Identifier: Name: %s Pod ID: %s", pod.Name, podIdentifier)
			continue
		}
	...
	err = r.updateeBPFMaps(ctx, podIdentifier, ingressRules, egressRules, ebpf.POLICIES_APPLIED)
	if err != nil {
		clog.Errorf("Map updates failed for podIdentifier %s: %v", podIdentifier, err)
		return err
	}
	return nil
}
...
func (r *PolicyEndpointsReconciler) cleanupPod(ctx context.Context, targetPod npatypes.Pod,
	...
	if _, ok := r.podIdentifierToPolicyEndpointMap.Load(podIdentifier); ok {
		...
		if err != nil {
			clog.Errorf("Error Parsing policy Endpoint resource %s: %v", policyEndpoint, err)
			return err
		}
		...
		if noActiveIngressPolicies && noActiveEgressPolicies {
			...
			clog.Infof("No active policies. Updating pod_state map for podIdentifier: %s networkPolicyMode: %s", podIdentifier, r.GeteBPFClient().GetNetworkPolicyMode())
			err = r.updateeBPFMaps(ctx, podIdentifier, ingressRules, egressRules, state)
			if err != nil {
				clog.Errorf("Map update(s) failed for podIdentifier %s: %v", podIdentifier, err)
				return err
			}
		} else {
			// We've additional PolicyEndpoint resources configured against this pod
			// Update the Maps and move on
			clog.Infof("Active policies against this pod. Skip Detaching probes and Update Maps... ")
...
func (r *PolicyEndpointsReconciler) deriveIngressAndEgressFirewallRules(ctx context.Context,
	...
	if policyEndpointList, ok := r.podIdentifierToPolicyEndpointMap.Load(podIdentifier); ok {
		clog.Infof("Total number of PolicyEndpoint resources for podIdentifier %s are %d", podIdentifier, len(policyEndpointList.([]string)))
		for _, policyEndpointResource := range policyEndpointList.([]string) {
			...
			if isDeleteFlow {
				...
				if deletedPEParentNPName == currentPEParentNPName {
					clog.Debugf("PE belongs to same NP. Ignore and move on since it's a delete flow deletedPE %s currentPE %s", resourceName, policyEndpointResource)
					continue
				}
			}
			...
			for _, endPointInfo := range currentPE.Spec.Egress {
				if endPointInfo.CIDR == "" && endPointInfo.DomainName == "" {
					clog.Warnf("both CIDR and DomainName are empty in PE name %s, NS: %s", currentPE.Name, currentPE.Namespace)
					continue
				}
				if endPointInfo.CIDR == "" {
					clog.Infof("CIDR is empty, skipping the egress rule %s, NS: %s", currentPE.Name, currentPE.Namespace)
					continue
				}
			...
			clog.Infof("Total no.of - ingressRules %d egressRules %d", len(ingressRules), len(egressRules))
...
func (r *PolicyEndpointsReconciler) deriveDefaultPodIsolation(ctx context.Context, policyEndpoint *policyk8sawsv1.PolicyEndpoint,
	...
	for _, value := range policyEndpoint.Spec.PodIsolation {
		if value == networking.PolicyTypeIngress && ingressRulesCount == 0 {
			clog.Info("Default Deny enabled on Ingress")
			isIngressIsolated = true
		}
		if value == networking.PolicyTypeEgress && egressRulesCount == 0 {
			clog.Info("Default Deny enabled on Egress")
...
func (r *PolicyEndpointsReconciler) updateeBPFMaps(ctx context.Context, podIdentifier string,
	...
	err := r.ebpfClient.UpdateEbpfMaps(ctx, podIdentifier, ingressRules, egressRules)
	if err != nil {
		clog.Errorf("Map update(s) failed for podIdentifier %s: %v", podIdentifier, err)
		return err
	}

	err = r.GeteBPFClient().UpdatePodStateEbpfMaps(ctx, podIdentifier, ebpf.POD_STATE_MAP_KEY, state, true, true)
	if err != nil {
		clog.Errorf("Map update(s) failed for podIdentifier %s: %v", podIdentifier, err)
		return err
	}

	r.ebpfClient.CreatePodStateEbpfEntryIfNotExists(ctx, podIdentifier, ebpf.CLUSTER_POLICY_POD_STATE_MAP_KEY, ebpf.DEFAULT_ALLOW)
	return nil
}
...
func (r *PolicyEndpointsReconciler) deriveTargetPodsForParentNP(ctx context.Context,
	...
	clog.Infof("Parent NP resource: Name: %s Total PEs for Parent NP: Count: %d", parentNP, len(parentPEList))
	...
	if podsPresent {
		...
		for _, pods := range existingPodsSlice {
			...
			clog.Infof("Current pods for this slice : Pod name %s Pod namespace %s", pods.Name, pods.Namespace)
		}
	}
	...
	if len(parentPEList) == 0 {
		...
		r.policyEndpointSelectorMap.Delete(policyEndpointIdentifier)
		clog.Infof("No PEs left: number of pods to cleanup - %d", len(podsToBeCleanedUp))
	}
	...
	for _, policyEndpointResourceName := range parentPEList {
		...
		clog.Infof("Processing PE Name %s", policyEndpointResourceName)
		currentTargetPods, currentPodIdentifiers := r.deriveTargetPods(ctx, currentPE, parentPEList)
		clog.Infof("Adding to current targetPods Total pods: %d", len(currentTargetPods))
	...
	for _, policyEndpointResource := range parentPEList {
		policyEndpointIdentifier := utils.GetPolicyEndpointIdentifier(policyEndpointResource,
			resourceNamespace)
		if len(targetPods) > 0 {
			clog.Infof("Update target pods for PE Object Name %s with Total pods: %d", policyEndpointResource, len(targetPods))
			r.policyEndpointSelectorMap.Store(policyEndpointIdentifier, targetPods)
		} else {
			clog.Infof("No more target pods so deleting the entry in PE selector map for Name %s", policyEndpointResource)
			r.policyEndpointSelectorMap.Delete(policyEndpointIdentifier)
		}
	...
	if len(currentPods) > 0 {
		podsToBeCleanedUp = r.getPodListToBeCleanedUp(ctx, currentPods, targetPods, podIdentifiers)
	}
...
func (r *PolicyEndpointsReconciler) deriveTargetPods(ctx context.Context,
	...
	for _, pod := range policyEndpoint.Spec.PodSelectorEndpoints {
		if string(pod.PodIP) == string(pod.HostIP) {
			clog.Infof("Skipping hostNetwork pod: name: %s namespace: %s (PodIP == HostIP: %s)",
		...
		if nodeIP.Equal(net.ParseIP(string(pod.HostIP))) {
			...
			podIdentifiers[podIdentifier] = true
			clog.Infof("Found a matching Pod: name: %s namespace: %s podIdentifier: %s", pod.Name, pod.Namespace, podIdentifier)
		}
...
func (r *PolicyEndpointsReconciler) getPodListToBeCleanedUp(ctx context.Context, oldPodSet []npatypes.Pod,
	...
	for _, oldPod := range oldPodSet {
		...
		if !activePod && !podIdentifiers[oldPodIdentifier] {
			clog.Infof("Pod to cleanup: name: %s namespace: %s", oldPod.Name, oldPod.Namespace)
...
func (r *PolicyEndpointsReconciler) updatePodIdentifierToPEMap(ctx context.Context, podIdentifier string,
	...
	if currentPESet, ok := r.podIdentifierToPolicyEndpointMap.Load(podIdentifier); ok {
		...
		for _, policyEndpointResourceName := range parentPEList {
			...
			if addPEResource {
				clog.Debugf("Adding PE name %s for podIdentifier %s", policyEndpointResourceName, podIdentifier)
...
func (r *PolicyEndpointsReconciler) derivePolicyEndpointsOfParentNP(ctx context.Context, parentNP, resourceNamespace string) []string {
	...
	if err := r.k8sClient.List(ctx, policyEndpointList, &client.ListOptions{
		...
		clog.Errorf("Unable to list PolicyEndpoints err: %v", err)
		return nil
	}
	...
	for _, policyEndpoint := range policyEndpointList.Items {
		if policyEndpoint.Spec.PolicyRef.Name == parentNP {
			...
			clog.Debugf("Found another PE resource for the parent NP name %s", policyEndpoint.Name)
		}
	}
...
func (r *PolicyEndpointsReconciler) DeriveFireWallRulesPerPodIdentifier(ctx context.Context, podIdentifier string, podNamespace string) ([]fwrp.EbpfFirewallRules,
	...
	ingressRules, egressRules, isIngressIsolated, isEgressIsolated, err := r.deriveIngressAndEgressFirewallRules(ctx, podIdentifier,
		podNamespace, "", false)
	if err != nil {
		clog.Errorf("Error deriving firewall rules: %v", err)
...
func (r *PolicyEndpointsReconciler) ArePoliciesAvailableInLocalCache(ctx context.Context, podIdentifier string) bool {
	if policyEndpointList, ok := r.podIdentifierToPolicyEndpointMap.Load(podIdentifier); ok {
		if len(policyEndpointList.([]string)) > 0 {
			clog := logger.FromContext(ctx)
			clog.Infof("Active policies available against podIdentifier %s", podIdentifier)
			return true
		}
	}
	return false
