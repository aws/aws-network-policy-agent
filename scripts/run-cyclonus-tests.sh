...
CGO_ENABLED=0 ginkgo -v -timeout 30m --no-color --fail-on-pending $GINKGO_TEST_BUILD_DIR/policy.test -- --cluster-kubeconfig=$KUBE_CONFIG_PATH --cluster-name=$CLUSTER_NAME --test-image-registry=$TEST_IMAGE_REGISTRY --ip-family=$IP_FAMILY || TEST_FAILED="true"
