...
WORKLOAD_PROFILE_ID=${WORKLOAD_PROFILE_ID:-npa-policy-churn-v3}
...
if [[ "$WORKLOAD_PROFILE_ID" != npa-policy-churn-v3 ]]; then
...
for ((round = 1; round <= SHORT_LIVED_ROUNDS; round++)); do
    ...
    WORKLOAD_SHORT_LIVED_POLICY_PROBES=$((WORKLOAD_SHORT_LIVED_POLICY_PROBES + SHORT_LIVED_TARGETS))
